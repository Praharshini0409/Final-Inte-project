...
function loadDashboard(interval) {
    ...
    if (fromDate && toDate) {
        url = `/api/dashboard?start_date=${fromDate}&end_date=${toDate}`;
    } else {
        // Update active button state
        const activeBtn = document.querySelector(`.time-btn[data-interval="${interval}"]`);
        if (activeBtn) {
            activeBtn.classList.add('active');
            console.log('Active button set to:', activeBtn.textContent.trim());
        } else {
            console.warn('Button not found for interval:', interval);
        }
    }
    ...
    fetch(url)
        ...
        .then(data => {
                        ...
                        x: { 
                            ...
                            ticks: { 
                                ...
                                callback: function(val, index) {
                                    ...
                                    return fullLabel.includes(' ') ? fullLabel.split(' ')[1] : fullLabel;
                                }
                            } 
                        },
            ...
            // Apply strikethrough styling to disabled legend items
            setTimeout(() => {
            ...
            // RE-APPLY ACTIVE BUTTON STYLING after chart finishes rendering
            // This ensures the button state is visible even after fetch completes
            setTimeout(() => {
                console.log('Re-applying button styling for interval:', currentInterval);
                document.querySelectorAll('.time-btn').forEach(btn => {
                    btn.classList.remove('active');
                });
                const activeBtn = document.querySelector(`.time-btn[data-interval="${currentInterval}"]`);
                if (activeBtn) {
                    activeBtn.classList.add('active');
                    console.log('Button re-highlighted:', activeBtn.textContent.trim());
                }
            }, 150);
        })
        .catch(err => {
...
function updateDateRange() {
    ...
    if (from && to) {
        loadDashboard(0); // Interval handled inside by querying dates
    }
}
...
function sendChat() {
    ...
    // Use the comprehensive local bot logic to instantly process
    setTimeout(() => {
